...
var firstNameStartingWithR = people
	.Where(x => x.FirstName.StartsWith('R')).ToList();
var orderByAgeDescending = (firstNameStartingWithR.OrderByDescending(x => x.Age));

var brownDogs = dogs
	.Where(x => x.Color == "Brown")
	.Where(x => x.Age >3).ToList();
var dogsByAge = brownDogs.OrderBy(x => x.Age);

var personWith2DogsOrMore = people
	.Where(x => x.Dogs.Count > 2);
var personWith2DogsOrMoreByName = personWith2DogsOrMore.OrderByDescending(x => x.FirstName);

var Freddy = people
	.FirstOrDefault(x => x.FirstName == "Freddy");
var FreddiesDogs = Freddy.Dogs
	.Where(x=> x.Age >1).ToList();

var Nathen = people.FirstOrDefault(x => x.FirstName == "Nathen");
var NathensFirstDog = Nathen.Dogs.First();

var peopleWithDogs = people
	.Where(x => x.FirstName == "Freddy" || x.FirstName == "Cristofer" || x.FirstName == "Erin" || x.FirstName == "Amelia").ToList();	
peopleWithDogs.OrderBy(x => x.FirstName).ToList();
var WhiteDogs = peopleWithDogs.SelectMany(x => x.Dogs.Where(x => x.Color == "White")).ToList();
WhiteDogs.OrderBy(x => x.Name);
